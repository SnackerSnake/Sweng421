...
namespace SWENG421FinalProject.Models.Prototype
{
    public class Protagonist : Character
    {
        ...
        public Protagonist(string name, int maxHealth, ReadWriteLock.ReadWriteLock lockManager) : base(name, maxHealth, lockManager)
        {
            this.Name = name;
        ...
        public override void SetHealth(int health)
        {
            try
            {
                ...
                if (health > 0)
                {
                    this.CurrHealth = this.CurrHealth + health >= this.MaxHealth ? this.MaxHealth : this.CurrHealth + health;
                }
                else
                {
        ...
        {
            if (!this.Items.Any(x => x.GetItemType() == (int)ItemTypeEnum.Weapon) && i.GetItemType() == (int)ItemTypeEnum.Weapon)
            {
                this.CurrItem = i;
            }
            this.Items.Add(i);
        ...
        {
            Protagonist tmp = (Protagonist)base.MemberwiseClone();
